...
class CtlToDatIo extends Bundle()
{
   ...
   val fencei     = Output(Bool())    // fencei instruction to stall the pipeline and let the I$ catch up

   val pipeline_kill = Output(Bool()) 
   val mem_exception = Output(Bool()) // csr handling an exception
...
// ------------------------------------- IF/ID-------------------------------------
 // Instruction Decode Stage from Signals
   val csignals =
...
// --------------------------- ID/EX -------------------------------
// following is the branch resolution logic for the branch instructions

   val ctrl_dec_pc_sel = Mux(io.ctl.pipeline_kill         , PC_EXC,
                           Mux(io.dat.dec_br_type === BR_J  , PC_BRJMP,
                         Mux(io.dat.dec_br_type === BR_N  , PC_4,
                         Mux(io.dat.dec_br_type === BR_NE , Mux(!io.dat.dec_br_eq,  PC_BRJMP, PC_4),
                         Mux(io.dat.dec_br_type === BR_EQ , Mux( io.dat.dec_br_eq,  PC_BRJMP, PC_4),
                         Mux(io.dat.dec_br_type === BR_LT , Mux( io.dat.dec_br_lt,  PC_BRJMP, PC_4),
                         Mux(io.dat.dec_br_type === BR_LTU, Mux( io.dat.dec_br_ltu, PC_BRJMP, PC_4),
                         Mux(io.dat.dec_br_type === BR_GE , Mux(!io.dat.dec_br_lt,  PC_BRJMP, PC_4),
                         Mux(io.dat.dec_br_type === BR_GEU, Mux(!io.dat.dec_br_ltu, PC_BRJMP, PC_4),
                         Mux(io.dat.dec_br_type === BR_JR , PC_JALR,
                                                            PC_4
                     ))))))))))


   val ifkill  = (ctrl_dec_pc_sel =/= PC_4) || cs_fencei || RegNext(cs_fencei)
...
// -------------------------------------------- EX/MEM -------------------------------
   ...
   io.ctl.dec_stall  := stall //  pipeline hazard: stall IF/DEC stages
   io.ctl.full_stall := full_stall //cache miss: stall entire pipeline and wait for D$ to catch up
   ...
   io.ctl.mem_exception := RegNext((exe_reg_illegal || io.dat.exe_inst_misaligned || io.dat.dec_inst_misaligned) && !io.dat.csr_eret) || io.dat.mem_data_misaligned 

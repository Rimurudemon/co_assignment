...
class DatToCtlIo(implicit val conf: SodorCoreParams) extends Bundle()
{
   ...
   val dec_br_type = Output(UInt(4.W))
   val exe_inst_misaligned = Output(Bool())
   val dec_inst_misaligned = Output(Bool())
...
class DatPath(implicit val p: Parameters, val conf: SodorCoreParams) extends Module
{
   ...
   io := DontCare
   val bit_pred = RegInit(0.U(2.W)) 
   val mem_tval_data_ma = Wire(UInt(conf.xprlen.W))
   val mem_tval_inst_ma = Wire(UInt(conf.xprlen.W))

// following are the state registers for the pipeline

   // Instruction Fetch State
   val if_reg_pc             = RegInit(io.reset_vector)

   // Instruction Decode State
   val dec_reg_valid         = RegInit(false.B) // If IF Still holds a valid instruction
   val dec_reg_inst          = RegInit(BUBBLE)  // The instruction
   val dec_reg_pc            = RegInit(0.asUInt(conf.xprlen.W)) // PC of the instruction
   val dec_reg_ctrl_br_type  = RegInit(BR_N)

   // Execute State
   // for instruction and validity ...
   ...
   // Instruction Fetch Stage :::: added the modification for branch predictor
   val if_pc_next          = Wire(UInt(32.W))
   val exe_brjmp_target    = Wire(UInt(32.W))
   val exe_jump_reg_target = Wire(UInt(32.W))
   val dec_jump_reg_target = Wire(UInt(32.W))
   val exception_target    = Wire(UInt(32.W))
   val dec_brjmp_target = Wire(UInt(32.W))
   ...
   if_pc_next := Mux(io.ctl.dec_pc_sel === PC_4,      if_pc_plus4,
                 Mux(io.ctl.dec_pc_sel === PC_BRJMP,  exe_brjmp_target,
                 Mux(io.ctl.dec_pc_sel === PC_JALR,  exe_jump_reg_target,
                 /*Mux(io.ctl.dec_pc_sel === PC_EXC*/ exception_target)))

   // Branch Predictor logic combined with the branch predictor state
   when (if_pc_next === exe_brjmp_target) { // Branch taken
   bit_pred := Cat(bit_pred(0), 1.U(1.W)) // Shift left, append 1
   } .otherwise { // Branch not taken
   bit_pred := Cat(bit_pred(0), 0.U(1.W)) // Shift left, append 0
   }
   when( io.ctl.dec_pc_sel === PC_BRJMP) {
      printf("Branch Predictor State: %b\n", bit_pred)
   }
   // for a fencei, refetch the if_pc (assuming no stall, no branch, and no exception)
   when (io.ctl.fencei && io.ctl.dec_pc_sel === PC_4 &&
   ...
   val rf_rs1_data = regfile.io.rs1_data // data in rs1
   val rf_rs2_data = regfile.io.rs2_data // data at rs2
   ...
   // Operand 2 Mux we are choosing what to consider as our op2
...
/////////// checking if the branch is taken or not------------------------


   dec_brjmp_target := dec_reg_pc + dec_op2_data
   dec_jump_reg_target := (dec_op1_data.asUInt + dec_op2_data.asUInt)(conf.xprlen-1,0) & ~1.U(conf.xprlen.W)
   io.dat.dec_inst_misaligned := (dec_brjmp_target(1, 0).orR    && io.ctl.dec_pc_sel === PC_BRJMP) ||
                                    (dec_jump_reg_target(1, 0).orR && io.ctl.dec_pc_sel === PC_JALR)
      mem_tval_inst_ma := RegNext(Mux(io.ctl.dec_pc_sel === PC_BRJMP, dec_brjmp_target, dec_jump_reg_target))
...
//**********************************the actual comparison logic for the branch**********************************
   ...
   io.dat.exe_inst_misaligned := (exe_brjmp_target(1, 0).orR    && io.ctl.dec_pc_sel === PC_BRJMP) ||
                                 (exe_jump_reg_target(1, 0).orR && io.ctl.dec_pc_sel === PC_JALR)
   mem_tval_inst_ma := RegNext(Mux(io.ctl.dec_pc_sel === PC_BRJMP, exe_brjmp_target, exe_jump_reg_target))
...
// the code for moving the actual hardware for the branch taken or not in the execute stage (MARK1 change)
// // exe_reg_op1_data <=> dec_op1_data
// // exe_reg_rs2_data <=> dec_rs2_data
...
//    io.dat.dec_br_type:= dec_reg_ctrl_br_type
   ...
   printf("Cyc= %d [%d] pc=[%x] W[r%d=%x][%d] Op1=[r%d][%x] Op2=[r%d][%x] inst=[%x] %c%c%c DASM(%x)\n",
      ...
      MuxLookup(io.ctl.dec_pc_sel, Str("?"))(Seq(
         PC_BRJMP -> Str("B"),
         PC_JALR -> Str("R"),
         PC_EXC -> Str("E"),
         PC_4 -> Str(" "))),

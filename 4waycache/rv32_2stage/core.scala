...
class Core(implicit val p: Parameters, val conf: SodorCoreParams) extends AbstractCore with MemoryOpConstants {
  ...
  // Connect CPU interface to Cache
  cache.io.cpu_req := c.io.dmem.req.valid                // Cache request valid if dmem request is valid
  cache.io.cpu_addr := c.io.dmem.req.bits.addr            // Cache address from dmem request address
  cache.io.cpu_wr := (c.io.dmem.req.bits.fcn === M_XWR)   // Write if function is write (M_XWR)
  cache.io.cpu_wdata := d.io.dmem.req.bits.data           // Data to be written from DatPath

  // Connect Cache output to DatPath
  d.io.dmem.resp.valid := cache.io.cpu_valid             // Cache response valid goes to DatPath response valid
  d.io.dmem.resp.bits.data := cache.io.cpu_rdata         // Cache read data goes to DatPath response data

  // Connect Cache memory interface to main memory (io.dmem)
  ...
  // Connect Cache memory response to Cache
  ...
  // Connect ready signal for the control path
